...
export async function POST(req: Request) {
  ...
  try {
    ...
    const facts = [
      `Launch on the table: token "${name}" (${symbol}), Uniswap V4 pool paired with ${pair.symbol} (${pair.name}) on Base, deployed by Bankr via Doppler${ownKey ? " from the human's own Bankr wallet" : " from the Bankr wallet on this install"}. Trading fees: 95% to the human's wallet ${feeRecipient.slice(0, 6)}…${feeRecipient.slice(-4)}, 5% to Bankr. ${ownKey ? "The human's own Bankr wallet deploys; creator vesting 15% stays with the human." : `The Bankr wallet on this install (${wallet ? `${wallet.evm.slice(0, 6)}…${wallet.evm.slice(-4)}` : "none"}) deploys on the human's behalf with creator vesting disabled, so the deployer keeps no tokens and 100% of the supply goes to the pool.`}`,
