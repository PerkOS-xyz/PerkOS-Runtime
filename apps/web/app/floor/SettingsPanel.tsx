...
export default function SettingsPanel({ onClose, debug, onDebug, perkos, onReconnectPerkos, rail, onLinkRail, desk, onLogout }: {
  ...
  const [guest, setGuest] = useState<{ invited: boolean; agentName?: string; status?: string; prompt?: string } | null>(null);
  ...
  const refresh = () =>
    Promise.all([
      ...
      fetch("/api/fleet/guest").then((r) => r.json()).then((j: { invited?: boolean; agentName?: string; status?: string; prompt?: string }) => setGuest({ invited: j.invited === true, agentName: j.agentName, status: j.status, prompt: j.prompt })).catch(() => setGuest({ invited: false }))
    ]);
  ...
  const mintGuest = async () => {
    ...
    try {
      ...
      const j = (await r.json().catch(() => ({}))) as { error?: string; invited?: boolean; agentName?: string; status?: string; prompt?: string; already?: boolean; ok?: boolean };
      if (!r.ok) { setGuest({ invited: false }); return; }
      setGuest({ invited: true, agentName: j.agentName, status: j.status, prompt: j.prompt });
      ...
      <form className="settings-dock" onSubmit={saveModel} aria-label="Settings">
        ...
        {desk ? (
          <>
            ...
            <p className="hint-line">Invite your Grok Bot onto this desk. Copy the prompt, paste it into Grok Bot. It drafts work with the team. You approve. Nothing spends until you hold.</p>
          </>
        ) : null}
