...
export function parseIntent(raw: string): Intent {
  ...
  if (/\b(what|which|que|qué|cual|cuál)\b.*\b(buy|invest|pick|comprar|invertir|conviene)\b|\b(worth buying|opportunit|oportunidad|best (stock|pick|buy)|take profit|ganancia|in a month|this month|para (un|1|el) mes|next month|one month|30 days)\b|\b(recommend|recomienda|recomiendas|aconseja)\b/.test(t) && !/\$\s*\d|\d+\s*(usd|usdc|dollars?)/.test(t) && !/\b(just|only)\s+advise\b/.test(t) && !/\badvise only\b/.test(t) && !/\b(i|they|we)\s+only\s+advise\b/.test(t) && !/\bdo not buy\b/.test(t)) return { kind: "advise" };
  ...
  if (/^(new chat|new conversation|start over|nuevo chat|nueva conversaci[oó]n|chat nuevo)$/.test(t) || /\b(start|open|begin|abre|empieza|inicia)\b.*\b(new|nuevo|nueva)\b.*\b(chat|conversation|conversaci[oó]n|thread|hilo)\b/.test(t)) return { kind: "newchat" };
  if (/\b(chat history|my chats|show (my |the )?chats|conversations|past chats|threads|mis chats|historial de chat|conversaciones|hilos)\b|^chats$/.test(t)) return { kind: "chats" };
