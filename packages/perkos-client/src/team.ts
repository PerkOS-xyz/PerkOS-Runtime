...
function toTeam(body: Record<string, unknown>, templateId: string): DeskTeam {
  ...
  const agents = rows.flatMap((a): TeamAgent[] => {
    ...
    return [{ role: a.role, name: a.name, state: a.state as TeamAgentState, ...(typeof a.detail === "string" ? { detail: a.detail } : {}) }];
  });
...
export class Team {
  ...
  async status(templateId: string): Promise<DeskTeam> {
  ...
  async wake(templateId: string): Promise<DeskTeam> {
